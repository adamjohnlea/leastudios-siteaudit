...
<div class="wrap">
	...
	<a href="<?php echo esc_url( $leastudios_siteaudit_create_url ); ?>" class="page-title-action"><?php esc_html_e( 'Add Project', 'leastudios-siteaudit' ); ?></a>
	<hr class="wp-header-end" />

	<?php if ( empty( $leastudios_siteaudit_projects ) ) : ?>
	...
	<?php else : ?>
		<table class="wp-list-table widefat striped">
			...
			<tbody>
				<?php foreach ( $leastudios_siteaudit_projects as $leastudios_siteaudit_project ) : ?>
					<?php
					...
					$leastudios_siteaudit_description = $leastudios_siteaudit_project->description();
					?>
					<tr>
						<td>
							<strong>
								<a href="<?php echo esc_url( $leastudios_siteaudit_edit_url ); ?>"><?php echo esc_html( $leastudios_siteaudit_project->name()->value() ); ?></a>
							</strong>
						</td>
						<td>
							<?php
							if ( null !== $leastudios_siteaudit_description && '' !== $leastudios_siteaudit_description ) {
								echo esc_html( wp_trim_words( $leastudios_siteaudit_description, 30, '…' ) );
							} else {
								echo '&mdash;';
							}
							?>
						</td>
						<td><?php echo esc_html( \LEAStudios\SiteAudit\Shared\Datetime_Util::format_immutable_for_display( $leastudios_siteaudit_project->created_at(), get_option( 'date_format', 'Y-m-d' ) ) ); ?></td>
						<td>
							...
							<?php if ( current_user_can( \LEAStudios\SiteAudit\Capabilities::MANAGE ) ) : ?>
								| <form method="post" action="<?php echo esc_url( $leastudios_siteaudit_delete_url ); ?>" style="display:inline" onsubmit="return confirm('<?php echo esc_js( __( 'Delete this project? URLs in it will become unassigned.', 'leastudios-siteaudit' ) ); ?>');">
									...
									<input type="hidden" name="id" value="<?php echo esc_attr( (string) $leastudios_siteaudit_project_id ); ?>" />
